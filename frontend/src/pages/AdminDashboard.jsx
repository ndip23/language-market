...
import { 
  ...
  Clock, Zap, DollarSign, Globe, ArrowRight 
} from 'lucide-react';
    ...
    <div className="min-h-screen bg-slate-50 pt-32 pb-20 px-4 md:px-12 animate-in fade-in duration-700">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row justify-between items-end mb-12 space-y-6">
            <div>
                <h1 className="text-4xl md:text-5xl font-black text-slate-900 tracking-tighter italic leading-none">System Root.</h1>
                <p className="text-slate-400 font-bold uppercase text-[9px] tracking-[0.3em] mt-3 italic underline decoration-emerald-500 decoration-4">Financials & User Oversight</p>
            </div>
            
            <div className="flex bg-white p-1.5 rounded-2xl shadow-sm border border-slate-100">
                <button onClick={() => setActiveTab('teachers')} className={`px-6 md:px-8 py-3 cursor-pointer rounded-xl font-black text-[10px] uppercase tracking-widest transition-all ${activeTab === 'teachers' ? 'bg-emerald-600 text-white shadow-lg shadow-emerald-500/20' : 'text-slate-400'}`}>Tutors</button>
                <button onClick={() => setActiveTab('students')} className={`px-6 md:px-8 py-3 cursor-pointer rounded-xl font-black text-[10px] uppercase tracking-widest transition-all ${activeTab === 'students' ? 'bg-emerald-600 text-white shadow-lg shadow-emerald-500/20' : 'text-slate-400'}`}>Learners</button>
            </div>
        </div>
          ...
          <table className="w-full text-left">
            <thead className="bg-slate-50 border-b border-slate-100 text-slate-400 font-black uppercase text-[9px] tracking-widest">
              <tr>
                ...
                <th className="px-6 py-8">{activeTab === 'teachers' ? 'Plan Status' : 'Active Connections'}</th>
            ...
            <tbody className="divide-y divide-slate-50">
              {(activeTab === 'teachers' ? data.teachers : data.students).map(u => (
                <tr key={u._id} className="hover:bg-slate-50/50 transition-colors group">
                  <td className="px-6 md:px-10 py-8 flex items-center space-x-4">
                    <img src={u.profilePicture || `https://ui-avatars.com/api/?name=${u.name}&background=10b981&color=fff`} className="w-12 h-12 rounded-2xl object-cover shadow-sm shrink-0" />
                  ...
                  <td className="px-6 py-8 text-center font-black text-slate-900 text-xs italic"><Globe size={12} className="inline mr-1 text-emerald-500"/> {u.countryCode}</td>
                  <td className="px-6 py-8">
                    {activeTab === 'teachers' ? (
                        u.subscription?.plan !== 'none' ? (
                            <span className="bg-emerald-50 text-emerald-600 px-4 py-1.5 rounded-full font-black text-[8px] uppercase tracking-widest border border-emerald-100 italic">
                                Paid: {u.subscription.plan}
                            </span>
                        ) : (
                            <span className="bg-amber-50 text-amber-600 px-4 py-1.5 rounded-full font-black text-[8px] uppercase tracking-widest border border-amber-100 animate-pulse">Unpaid</span>
                        )
                    ) : (
                        ...
                        <div className="flex flex-wrap gap-2">
                            {u.bookedTutors && u.bookedTutors.length > 0 ? (
                                u.bookedTutors.map((c, i) => (
                                    <div key={i} className={`flex items-center px-3 py-1 rounded-lg border text-[8px] font-black uppercase tracking-tighter ${c.isPaid ? 'bg-emerald-50 text-emerald-600 border-emerald-100' : 'bg-slate-50 text-slate-400 border-slate-200'}`}>
                                        <span>{c.teacher?.name}</span>
                                        {c.isPaid ? <CheckCircle2 size={8} className="ml-1.5"/> : <Clock size={8} className="ml-1.5"/>}
                                    </div>
                                ))
                            ) : (
                  ...
                  <td className="px-6 md:px-10 py-8 text-right">
                    {activeTab === 'teachers' ? (
                        <button onClick={() => handleApprove(u._id, u.teacherProfile.isApproved)}
                            className={`px-8 py-3 rounded-full font-black text-[10px] uppercase tracking-widest shadow-lg transition-all ${u.teacherProfile.isApproved ? 'bg-red-50 text-red-500 hover:bg-red-500 hover:text-white border border-red-100' : 'bg-emerald-600 text-white hover:bg-slate-900 shadow-emerald-500/20'}`}
                        >
                            {u.teacherProfile.isApproved ? 'Suspend' : 'Approve'}
                        </button>
                    ) : (
                        <button className="px-8 py-3 rounded-full font-black text-[10px] uppercase tracking-widest text-slate-400 border border-slate-100 hover:bg-slate-50">Details</button>
                    )}
                  </td>
                </tr>
              ))}
